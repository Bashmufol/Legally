...
function mediaLabel(mime: string): string {
  if (mime.startsWith('audio/')) return 'Voice recording'
...
interface Props {
  ...
  /** Hide items from the list (e.g. audio shown by VoiceRecorder). Removal still uses original index. */
  hideMimePrefix?: string
}

export default function AttachedMediaList({ items, onRemove, hideMimePrefix }: Props) {
  const visible = items
    .map((item, index) => ({ item, index }))
    .filter(({ item }) => !hideMimePrefix || !item.mimeType.startsWith(hideMimePrefix))
      ...
      {visible.map(({ item, index }) => (
        ...
        >
          ...
          <button
            ...
            aria-label={`Remove ${mediaLabel(item.mimeType).toLowerCase()}`}
          >
            <X className="w-4 h-4" />
          </button>
        </li>
