...
interface Props {
  ...
  hasAttachedVoice?: boolean
}

export default function VoiceRecorder({ onRecorded, disabled, onBusyChange, hasAttachedVoice }: Props) {
  ...
  useEffect(() => {
    if (!recording && hasAttachedVoice === false && status === 'Voice note attached') {
      setStatus(null)
    }
  }, [hasAttachedVoice, recording, status])
