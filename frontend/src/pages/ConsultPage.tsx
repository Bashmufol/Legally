...
export default function ConsultPage() {
  ...
  const { jurisdiction, hasDeviceLocation } = useJurisdiction()
  ...
  const hasAttachedVoice = media.some((m) => m.mimeType.startsWith('audio/'))
  ...
  const canSubmit =
    hasConsultInput(message, media) && !loading && !mediaBusy && !waitingForAuth
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10 w-full min-w-0">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="space-y-4">
          <div
            className="flex gap-3 rounded-lg border border-legally-gold/40 bg-legally-gold/10 px-4 py-3 text-sm text-legally-navy/85"
            role="status"
          >
            <Info className="w-5 h-5 shrink-0 text-legally-gold mt-0.5" aria-hidden />
            <p>
              {hasDeviceLocation ? (
                <>
                  Using device location:{' '}
                  <span className="font-medium text-legally-navy">
                    {jurisdiction.regionName
                      ? `${jurisdiction.countryName}, ${jurisdiction.regionName}`
                      : jurisdiction.countryName}
                  </span>
                  . Mention another country or state in your description to override.
                </>
              ) : (
                <>Mention a country or state in your description to set jurisdiction.</>
              )}
            </p>
          </div>
          <textarea
          ...
          <VoiceRecorder
            ...
            disabled={loading}
            onBusyChange={setMediaBusy}
            hasAttachedVoice={hasAttachedVoice}
          />
          ...
          {mediaBusy && (
            <p className="text-xs text-legally-navy/60 text-center">Finishing upload…</p>
          )}
          <button
            type="button"
            onClick={submit}
          ...
          {!hasConsultInput(message, media) && !mediaBusy && (
